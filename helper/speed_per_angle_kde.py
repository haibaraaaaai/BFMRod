...
def analyze_file_histograms(npz_path):
    ...
    # --- Plot histogram + KDE for each angle bin ---
    for i, angle_start in enumerate(angle_bins):
        ...
        plt.figure()
        # Histogram
        plt.hist(speeds, bins=100, color='steelblue', alpha=0.7, density=True, label="Histogram")

        # KDE
        kde = gaussian_kde(speeds, bw_method=0.1)
        x_vals = np.linspace(0, MAX_SPEED_HZ, 500)
        kde_vals = kde(x_vals)
        plt.plot(x_vals, kde_vals, color='darkred', linewidth=2, label="KDE")
    ...
    print("✅ Histograms with KDE saved for all valid angular bins.")
